...
CONSTANTS
    ...
    \* ConflictPairs is used to define the conflict between transactions
...
VARIABLES
    ...
    crashedProcesses    \* crashedPrcesses[s][p] = true if that process has crashed

vars == << crashedProcesses, bal, phase, txn, dep, ts, abal, msgs, submitted, initTimestamp, initCoord, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, Qvar,  consumedMsgs >>
...
Init == 
    ...
    /\ consumedMsgs = {}
    /\ crashedProcesses = [s \in Shards |-> [p \in Proc |-> FALSE ]]
...
IsPartitionCoord(s, p, id) ==
    [proc |-> p, s |-> s] \in initCoord[id] 

initPartitionCoord(id, s) ==
    IF \E coord \in initCoord[id] : coord.shard = s
    THEN CHOOSE coord \in initCoord[id] : coord.shard = s
    ELSE [proc |-> NoProc, shard |-> 0]

NumberOfCrashedProcessesInShard(s) ==
    Cardinality({ p \in Proc : crashedProcesses[s][p] })
...
Submit(s, p, id) ==
    /\  id \notin submitted
    /\  s \in idToShard[id] 
    /\  LET initCoords == { [proc |-> p, shard |-> shard] : shard \in idToShard[id]}
            earlierInitTimestamps == {initTimestamp[id2] : id2 \in {id1 \in Id : initPartitionCoord(id1, s) = [shard |-> s, proc |-> p] /\ LessThanTs(initTimestamp[id], initTimestamp[id1])}}
        IN 
        ...
        /\  msgs' = msgs \cup { SubmitMsg(s, p, coord.shard, coord.proc, id) : coord \in initCoords }
    /\  UNCHANGED << crashedProcesses, bal, abal, txn, phase, ts, dep, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, Qvar,  consumedMsgs>> 
...
HandleSubmit(m) ==
    ...
    /\  LET s  == m.shardto
        ...
        IN 
        (* /\  \A id2 \in Id : 
            (phase[s][p][id2] = InitialPhase /\ Conflicts(id, id2)) => LessThanTs(ts[s][p][id2], initTimestamp[id]) *)
        /\  LET computations == PreAcceptComputations(s, p, s, p, id, initTimestamp[id])
            ...
            /\  msgs' = (msgs \ {m}) \cup { PreAcceptMsg(sq, q, s, to, id, tx, computations.D) : to \in Proc \ {p} } 
                                     \cup { PreAcceptOKMsg(sq, q, s, p, id, computations.finalTs, computations.D) }
    /\  UNCHANGED << crashedProcesses, initTimestamp, submitted, initCoord, bal, abal, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, Qvar,  consumedMsgs>> 
...
HandlePreAccept(m) ==
    ...
    /\  UNCHANGED << crashedProcesses, bal, abal, submitted, initCoord, recovered, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, Wvar, Qvar, initTimestamp>>
...
HandlePreAcceptOK(s, p, id) ==
    ...
    /\  LET  quorumOfMessages ==
        ...
        /\  LET largestFastQuorum ==
            ...
            IF IsFastQuorum(largestFastQuorum, id) THEN
                    ...
                    /\  UNCHANGED << crashedProcesses, bal, abal, txn>>
    ...
    /\  UNCHANGED << crashedProcesses, submitted, initCoord, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, initTimestamp, Qvar>>
...
HandlePreCommit(m) ==
    ...
    /\  LET s == m.shardto
        ...
        /\  consumedMsgs' = consumedMsgs \cup {m}
    /\  UNCHANGED << crashedProcesses, bal, abal, txn, submitted, initCoord, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, initTimestamp, Qvar>>
...
HandlePreCommitOK(s, p, id) ==
    ...
    /\  UNCHANGED << crashedProcesses, submitted, initTimestamp, initCoord, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, Qvar >>
    ...
    /\  LET quorumOfMessages == 
        ...
        /\  LET D == dep[s][p][id] \cup UNION { m.body.Dq : m \in quorumOfMessages }
            ...
            ELSE 
                ...
                /\  consumedMsgs' = consumedMsgs \cup quorumOfMessages
    /\  UNCHANGED << crashedProcesses, bal, submitted, initCoord, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, initTimestamp, Qvar >>
...
HandleCommit(m) ==
    ...
    /\  LET s == m.shardto
        ...
        /\  IF (pathSpeed = Fast /\ b = 0 /\ IsPartitionCoord(s, p, id)) THEN 
            ...
            ELSE msgs' = msgs \ {m}
        /\  consumedMsgs' = consumedMsgs \cup {m}
    /\  UNCHANGED << crashedProcesses, bal, submitted, initCoord, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, Qvar,  initTimestamp>>
...
HandleCommitOK(s, p, id) ==
    ...
    /\  LET quorumOfMessages == 
        ...
        /\  consumedMsgs' = consumedMsgs \cup quorumOfMessages
    /\  UNCHANGED << crashedProcesses, bal, txn, dep, ts, abal, submitted, initCoord, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, initTimestamp, Qvar>>
...
HandleStable(m) ==
    ...
    /\  LET s == m.shardto
        ...
        /\  msgs' = msgs \ {m}
        /\  consumedMsgs' = consumedMsgs \cup {m}
        /\  UNCHANGED << crashedProcesses, bal, submitted, initCoord, dep, abal, txn, ts, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, initTimestamp, Qvar>>
...
/\ UNCHANGED << crashedProcesses, phase, dep, ts, abal, submitted, initCoord, Wvar, TXvar, Dvar, initTimestamp, Qvar, consumedMsgs, recoveryAttemptBal>>
...
HandleRecover(m) ==
    ...
    /\  LET s == m.shardto
        ...
        /\  LET computations == RecoverComputations(s, p, id)
            ...
            /\  consumedMsgs' = consumedMsgs \cup {m}
    /\  UNCHANGED << crashedProcesses, submitted, initCoord, dep, abal, ts, phase, recovered, TXvar, Dvar, postWaitingFlag, Wvar, recoveryAttemptBal, initTimestamp, Qvar>>
            ...
            /\  IF (\E n \in U : n.body.phaseq  = StablePhase)
                THEN
                        /\  LET n == CHOOSE n \in U :
                            ...
                            /\  msgs' = (msgs \ quorumOfMessages) \cup { CommitMsg(s, p, to[1], to[2], bal[s][p][id], id, n.body.tq, n.body.depq, Fast, n.body.txq) : to \in { <<sq, q>> : sq \in idToShard[id], q \in Proc } \ { <<s, p>> } }
                                                                  \cup { StableMsg(s, p, to[1], to[2], bal[s][p][id], id) : to \in { <<sq, q>> : sq \in idToShard[id], q \in Proc } \ { <<s, p>> } }
                            /\  consumedMsgs' = consumedMsgs \cup quorumOfMessages
                            /\  UNCHANGED << crashedProcesses, bal, TXvar, Wvar, Dvar, recoveryAttemptBal, postWaitingFlag, Qvar>> 
                ...
                THEN    
                        /\  LET n == CHOOSE n \in U :
                            ...
                            /\  consumedMsgs' = consumedMsgs \cup quorumOfMessages
                            /\  UNCHANGED << crashedProcesses, TXvar, Wvar, Dvar, recoveryAttemptBal, postWaitingFlag, Qvar>> 
                ...
                ELSE IF ( \A shard \in idToShard[id] :
                        ...
                        ELSE 
                            ...
                            /\  consumedMsgs' = consumedMsgs \cup quorumOfMessages
                            /\  UNCHANGED << crashedProcesses, bal, txn, abal, ts, dep, phase>>
                ELSE  
                    LET computations == AcceptComputations(s, p, id, ts[s][p][id])
                    IN 
                    /\  ApplyAccept(s, p, bal[s][p][id], id, ts[s][p][id], dep[s][p][id], Nop)
                    ...
                    /\  consumedMsgs' = consumedMsgs \cup quorumOfMessages
                    /\  UNCHANGED << crashedProcesses, TXvar, Wvar, Dvar, recoveryAttemptBal, postWaitingFlag, Qvar>>   
    /\  UNCHANGED << crashedProcesses, submitted, initCoord, recovered, initTimestamp >>
...
HandlePostWaiting(s, p, id) ==
    ...
    /\  LET W == Wvar[s][p][id]
        ...
        \/  /\  ~Case1 /\ ~Case2 /\ ~Case3
            /\  UNCHANGED << crashedProcesses, msgs, postWaitingFlag, bal, dep, phase, abal, txn, ts>>
                      
    /\  UNCHANGED << crashedProcesses, submitted, initCoord, recovered, Wvar, recoveryAttemptBal, TXvar, Dvar, initTimestamp, Qvar, consumedMsgs>>


CrashProcess(s, p) ==
    /\ NumberOfCrashedProcessesInShard(s) < F
    /\ crashedProcesses' = [crashedProcesses EXCEPT ![s][p] = TRUE]
    /\ UNCHANGED << bal, phase, txn, dep, ts, abal, msgs, submitted, initTimestamp, initCoord, recovered, Wvar, postWaitingFlag, recoveryAttemptBal, TXvar, Dvar, Qvar,  consumedMsgs >>
...
WitnessSomeStable == ~SomeCommandStable

CommandSubmitted ==
    \E id \in Id : id \in submitted

WitnessSubmitted == ~CommandSubmitted 

W_PreAcceptMsgInFlight ==
    ~\E m \in msgs : m.type = TypePreAccept

W_SomePreAccepted ==
    ~\E id \in Id : \E s \in Shards : \E p \in Proc :
        phase[s][p][id] = PreAcceptedPhase
...
Next == \E s \in Shards, p \in Proc, id \in Id :
            /\  ~crashedProcesses[s][p] 
            /\  \/ HandleSubmitWrapper(s, p, id)
                \/ HandlePreAcceptWrapper(s, p, id) 
                \/ HandleAcceptWrapper(s, p, id)
                \/ HandleCommitWrapper(s, p, id)
